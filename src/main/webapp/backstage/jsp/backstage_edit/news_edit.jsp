...
<head>
  ...
  <title>${newsType.newsTypeName}-编辑-韶关钢铁网站管理平台</title>

   <%@include file="../share/share_css.jsp" %>
   <link rel="stylesheet" type="text/css" href="${basepath}/admin/css/bootstrap-fileupload.min.css" />
    ...
    <div class="main-content" >
       ...
       <%@include file="../share/header.jsp" %>
        ...
        <div class="page-heading" style="border-bottom: 1px solid #CCC">
            <h3>
               ${newsType.newsTypeName} <small>编辑列表</small>
            </h3>
...
<!--表格开始标志  -->   <form action="${basepath}/news/edit.do" method="post" onsubmit="return checkFrm();">
        ...
        <!--  newstypeId  -->    <input type="hidden"  name="newsType.newsTypeId" value="${news.newsType.newsTypeId}"/> 
        <!--  userId -->         <input type="hidden"   name="user.userId" value="${news.user.userId}"/>
        <!--  picId -->          <%-- <input type="hidden" id="test"  value="${news.pic.id}" name="pic.id" /> --%>
        ...
        <!--  author  -->        <input type="text" name="author"  value="${news.author}" class="form-control"  placeholder="文章作者" maxlength = "20">
                            ...
                            <c:if test="${newsType.newsTypeName eq '最新资讯'}">           
                            <hr>
                            <!--上传图片 start-->
...
function ajaxFileUpload()
{
    ...
    (
        {
            url:'${basepath}/file_upload.do?id=${user.userId}',//用于文件上传的服务器端请求地址
