                            ...
                            <table class="table  table-hover general-table">
                              ...
                              <c:forEach varStatus="varStatus" items="${pageNews.datas}" var="news">
                                 <tr bgcolor="${varStatus.index % 2 == 1 ? '#EFEFEF' : 'FFFFFF' }">  
                                 	<td><a href="" target="_blank">${varStatus.count}</a></td> 
            							...
            							<nav aria-label="...">
             							 <ul class="pagination">
                   							 <c:if test="${chooseNum>1}">
                          				 		<li><a href="${basepath}/backstage/${secondBar.newsTypeId}?chooseNum=1">«</a></li>
                      						 </c:if>
	                        	 <c:forEach begin="1" end="${pageNews.pageNum}" var="each"> 
                           			 <c:choose>
                              			  <c:when test="${chooseNum == each}">
                              	 		  <li class="active"><a href="${basepath}/backstage/${secondBar.newsTypeId}?chooseNum=${each}">${each}</a></li>
                              			  </c:when>
                              			  <c:when test="${each>=(chooseNum-2) && each<=(chooseNum+2)}">
                              	  		 <li><a href="${basepath}/backstage/${secondBar.newsTypeId}?chooseNum=${each}">${each}</a></li>
                              			</c:when>
                            		</c:choose>
                      			 </c:forEach>
                                <c:if test="${chooseNum < pageNews.pageNum}">   
                                  <li><a href="${basepath}/backstage/${secondBar.newsTypeId}?chooseNum=${pageNews.pageNum}">»</a></li>
                                 </c:if>
